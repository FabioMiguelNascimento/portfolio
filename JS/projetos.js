...
function criarHTMLProjeto(projeto, index) {
  ...
  function getIconPath(linguagem) {
    const nomeArquivo =
      linguagem.toLowerCase().replace(/[^a-z0-9]/g, "") + ".svg";
  ...
  const imageOrIframe = projeto.isFigma
    ...
    : `<a href="${
        projeto.links.find((link) => link.nome === "Site")?.url || "#"
      }" target="_blank">
...
<div class="box ${direcao} ${projeto.nome.replace(/\s+/g, "")}">
    ...
    <div class="detalhes">
        <div class="status-emoji tooltip">
            ...
            <span class="tooltiptext" data-translate="status_${projeto.status
              .toLowerCase()
              .replace(/\s+/g, "_")}">${projeto.status}</span>
        </div>
        <div class="nome">
            <h1 data-translate="project_name_${projeto.id}">${projeto.nome}</h1>
        </div>
        <div class="descricao">
            <p data-translate="project_description_${projeto.id}">${
    projeto.descricao
  }</p>
        </div>
        <ul class="techstack">
            ${
              projeto.linguagens
                ? projeto.linguagens
                    ...
                    <span class="tooltiptext" data-translate="tech_${lang
                      .toLowerCase()
                      .replace(/\s+/g, "_")}">${lang}</span>
                </li>`
                    )
                    .join("")
                : ""
            ...
            ${projeto.links
              .map(
                (link) =>
                  `<a href="${
                    link.url
                  }" target="_blank" class="projetos-botao" aria-label="${
                    link.nome
                  }">
                    <i class="${
                      link.nome.toLowerCase() === "site"
                        ? "fas fa-play"
                        : "fab fa-" + link.nome.toLowerCase()
                    } icon"></i>
                    <span data-translate="link_${link.nome.toLowerCase()}">${
                    link.nome
                  }</span>
                </a>`
              )
